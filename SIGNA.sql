...
-- Cria procedure SP_SELECTATRI_ESPECIE
DROP PROCEDURE IF EXISTS SP_SELECTATRI_ESPECIE
GO
CREATE PROCEDURE SP_SELECTATRI_ESPECIE
@ESPECIE INT = NULL
AS
IF (EXISTS (SELECT 1 FROM TBATRI_ESPECIE WHERE IDESPECIE = NULLIF(@ESPECIE,''))) -- Caso a espécie possua algum atributo com conteúdo
BEGIN
	--Exiba isso
	SELECT DISTINCT ATR.*,DESCRICAO FROM TBATRIBUTO ATR LEFT JOIN TBATRI_ESPECIE ASS ON ATR.IDATRIBUTO = ASS.IDATRIBUTO WHERE IDESPECIE = @ESPECIE OR IDESPECIE IS NULL
END
ELSE
BEGIN
	-- Caso contrário faça isso
	SELECT *,NULL DESCRICAO FROM TBATRIBUTO
END